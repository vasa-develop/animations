...
class Sequence14(Scene):
    ...
    def construct(self):
        ...
        dot_2 = Dot().set_color(GRAY).move_to(axes.c2p(3, 1))

        # Create label for y-intercept value
        dot_1_label = MathTex(r"(0,0)").next_to(dot_1, DOWN, buff=0.1)
        dot_2_label = MathTex(r"(3,1)").next_to(dot_2, UP, buff=0.1)

        line = Line(dot_1.get_center(), dot_2.get_center()).set_color(GREEN)
